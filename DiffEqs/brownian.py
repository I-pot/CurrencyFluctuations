#I guess I want experiment with geometric brownian motion before that. See
#https://www.quantstart.com/articles/geometric-brownian-motion-simulation-with-python/
#for an explanation on below.
#Call with:
#python3 diffEqs.py --num-assets=5 --random-seed=41 --start-date='1993-01-01' --end-date='2022-07-31' --output-dir='.' --symbol-length=5 --init-price=100.0 --mu=0.1 --sigma=0.3 --pareto-shape=1.5
import os
import random
import string
import click
...
if(False):
    ...
    class GeometricBrownianMotionAssetSimulator:
        ...
        def _create_empty_frame(self):
            ...
            return pd.DataFrame({'date': date_range,'open': zeros,'high': zeros,'low': zeros,'close': zeros,'volume': zeros})[['date', 'open', 'high', 'low', 'close', 'volume']]
...
if(False):
    tieNimi = input("Name of the file you want to plot? E.g. BUAWV \n")
    df = pd.read_csv(tieNimi+'.csv').set_index('date')
